...
在[4.5节](4.5.%20JSON)我们使用构体成员标签用于设置对应JSON对应的名字。其中json成员标签让我们可以选择成员的名字和抑制零值成员的输出。在本节，我们将看到如何通过反射机制类获取成员标签。
...
**练习 12.13：** 修改S表达式的编码器（[§12.4](12.4.%20示例：编码为S表达式)）和解码器（[§12.6](12.6.%20示例：解码S表达式)），采用和encoding/json包（[§4.5](4.5.%20JSON)）类似的方式使用成员标签中的sexpr:"..."字串。
