...
这就是三个帮助函数是怎么工作的。例如下面展示的IsNotExist，它会报出是否一个错误和syscall.ENOENT（[§7.8](7.8.%20error接口)）或者和有名的错误os.ErrNotExist相等（可以在[§5.4.2](5.4.%20错误#5.4.2.%20文件结尾错误（EOF）)中找到io.EOF）；或者是一个`*PathError`，它内部的错误是syscall.ENOENT和os.ErrNotExist其中之一。
