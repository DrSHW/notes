...
在[第一章](../第1章%20入门/本章概要)中，我们粗略的了解了怎么用net/http包去实现网络客户端（[§1.5](1.5.%20获取URL)）和服务器（[§1.7](1.7.%20Web服务)）。在这个小节中，我们会对那些基于http.Handler接口的服务器API做更进一步的学习：
...
/price的case会调用URL的Query方法来将HTTP请求参数解析为一个map，或者更准确地说一个net/url包中url.Values([§6.2.1](6.2.%20基于指针对象的方法#6.2.1.%20Nil也是一个合法的接收器类型))类型的多重映射。然后找到第一个item参数并查找它的价格。如果这个货品没有找到会返回一个错误。
...
让我们关注这两个注册到handlers上的调用。第一个db.list是一个方法值（[§6.4](6.4.%20方法值和方法表达式)），它是下面这个类型的值。
...
最后，一个重要的提示：就像我们在[1.7节](1.7. Web服务)中提到的，web服务器在一个新的协程中调用每一个handler，所以当handler获取其它协程或者这个handler本身的其它请求也可以访问到变量时，一定要使用预防措施，比如锁机制。我们后面的两章中将讲到并发相关的知识。
...
**练习 7.12：** 修改/list的handler让它把输出打印成一个HTML的表格而不是文本。html/template包（[§4.6](4.6.%20文本和HTML模板)）可能会对你有帮助。
