...
调用errors.New函数是非常稀少的，因为有一个方便的封装函数fmt.Errorf，它还会处理字符串格式化。我们曾多次在[第五章](../第5章%20函数/本章概要)中用到它。
...
Errno是一个系统调用错误的高效表示方式，它通过一个有限的集合进行描述，并且它满足标准的错误接口。我们会在[7.11节](7.11.%20基于类型断言区别错误类型)了解到其它满足这个接口的类型。
