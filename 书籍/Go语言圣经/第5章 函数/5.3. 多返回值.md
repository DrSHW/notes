...
在findlinks中，有4处`return`语句，每一处return都返回了一组值。前三处return，将`http`和`html`包中的错误信息传递给findlinks的调用者。第一处return直接返回错误信息，其他两处通过`fmt.Errorf`（§7.8）输出详细的错误信息。如果findlinks成功结束，最后的return语句将一组解析获得的连接返回给用户。
...
**练习 5.6：** 修改gopl.io/ch3/surface（§3.2）中的`corner`函数，将返回值命名，并使用bare return。
