...
`visit`函数遍历HTML的节点树，从每一个anchor元素（\<a>标签）的`href`属性获得link,将这些links存入字符串数组中，并返回这个字符串数组：
...
让我们以Go的主页（golang.org）作为目标，运行`findlinks`。我们以fetch程序（1.5章）的输出作为`findlinks`的输入。下面的输出做了简化处理。
