...
到目前为止，我们所见到的卷积神经网络层，例如卷积层（ *小节《6.2 图像卷积.md》中*）和汇聚层（ *小节《6.5 汇聚层.md》中*），通常会减少下采样输入图像的空间维度（高和宽）。
...
例如， 图 13.10.1 解释了如何为$2\times 2$的输入张量计算卷积核为$2\times 2$的转置卷积。

<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/trans_conv.svg" alt="卷积核为 $2\times 2$ 的转置卷积。阴影部分是中间张量的一部分，也是用于计算的输入和卷积核张量元素。 " style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 13.10.1 卷积核为 $2\times 2$ 的转置卷积。阴影部分是中间张量的一部分，也是用于计算的输入和卷积核张量元素。 </p>
...
与通过卷积核“减少”输入元素的常规卷积（在 *小节《6.2 图像卷积.md》中*中）相比，转置卷积通过卷积核“广播”输入元素，从而产生大于输入的输出。
我们可以通过 图 13.10.1 来构建输入张量`X`和卷积核张量`K`从而[**验证上述实现输出**]。
此实现是基本的二维转置卷积运算。
...
使用 图 13.10.1 中相同输入和卷积核张量，将步幅从1更改为2会增加中间张量的高和权重，因此输出张量在 图 13.10.2 中。

<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/trans_conv_stride2.svg" alt="卷积核为$2\times 2$，步幅为2的转置卷积。阴影部分是中间张量的一部分，也是用于计算的输入和卷积核张量元素。" style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 13.10.2 卷积核为$2\times 2$，步幅为2的转置卷积。阴影部分是中间张量的一部分，也是用于计算的输入和卷积核张量元素。</p>
...
1. 在 *小节《13.10 转置卷积》中[**与矩阵变换的联系**]部分*中，卷积输入`X`和转置的卷积输出`Z`具有相同的形状。他们的数值也相同吗？为什么？
1. 使用矩阵乘法来实现卷积是否有效率？为什么？

关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/3301)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/3302)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11810)。
