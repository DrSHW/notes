...
如 *小节《13.9 语义分割和数据集.md》中*中所介绍的那样，语义分割是对图像中的每个像素分类。
*全卷积网络*（fully convolutional network，FCN）采用卷积神经网络实现了从图像像素到像素类别的变换 。
与我们之前在图像分类或目标检测部分介绍的卷积神经网络不同，全卷积网络将中间层特征图的高和宽变换回输入图像的尺寸：这是通过在 *小节《13.10 转置卷积.md》中*中引入的*转置卷积*（transposed convolution）实现的。
...
如 图 13.11.1 所示，全卷积网络先使用卷积神经网络抽取图像特征，然后通过$1\times 1$卷积层将通道数变换为类别个数，最后在 *小节《13.10 转置卷积.md》中*中通过转置卷积层将特征图的高和宽变换为输入图像的尺寸。
因此，模型输出与输入图像的高和宽相同，且最终输出通道包含了该空间位置像素的类别预测。

<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/fcn.svg" alt="全卷积网络" style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 13.11.1 全卷积网络</p>
...
回想一下 *小节《6.3 填充和步幅.md》中*中卷积层输出形状的计算方法：
...
```python
img = image.imread('C:/Users/17100/Desktop/知识库/深度学习方法/img/catdog.jpg')
...
```python
# 使用pytorch实现
img = torchvision.transforms.ToTensor()(d2l.Image.open('C:/Users/17100/Desktop/知识库/深度学习方法/img/catdog.jpg'))
X = img.unsqueeze(0)
Y = conv_trans(X)
...
```python
# 使用paddle实现
img = paddlevision.transforms.ToTensor()(d2l.Image.open('C:/Users/17100/Desktop/知识库/深度学习方法/img/catdog.jpg'))
X = img.unsqueeze(0)
Y = conv_trans(X)
...
除了坐标刻度不同，双线性插值放大的图像和在 *小节《13.3 目标检测和边界框.md》中*中打印出的原图看上去没什么两样。
...
我们用 *小节《13.9 语义分割和数据集.md》中*中介绍的语义分割读取数据集。
...
def load_data_voc(batch_size, crop_size):
    """加载VOC语义分割数据集
    Defined in *小节《13.9 语义分割和数据集.md》中*"""
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/3298)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/3297)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11811)。
