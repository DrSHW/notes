...
在 *小节《13.4 锚框.md》中*中，我们以输入图像的每个像素为中心，生成了多个锚框。
...
回想一下，在 *小节《6.2 图像卷积.md》中*中，我们将卷积图层的二维数组输出称为特征图。
...
下面，我们介绍一种基于CNN的多尺度目标检测方法，将在 *小节《13.7 单发多框检测（SSD）.md》中*中实现。

在某种规模上，假设我们有$c$张形状为$h \times w$的特征图。
使用 *小节《13.5 多尺度目标检测》中多尺度锚框部分*中的方法，我们生成了$hw$组锚框，其中每组都有$a$个中心相同的锚框。
例如，在 *小节《13.5 多尺度目标检测》中多尺度锚框部分*实验的第一个尺度上，给定10个（通道数量）$4 \times 4$的特征图，我们生成了16组锚框，每组包含3个中心相同的锚框。
...
根据 *小节《6.2 图像卷积.md》中*中对感受野的定义，特征图在相同空间位置的$c$个单元在输入图像上的感受野相同：
它们表征了同一感受野内的输入图像信息。
...
在 *小节《13.7 单发多框检测（SSD）.md》中*，我们将通过一个具体的例子来说明它是如何工作的。
...
1. 根据我们在 *小节《7.1 深度卷积神经网络（AlexNet）.md》中*中的讨论，深度神经网络学习图像特征级别抽象层次，随网络深度的增加而升级。在多尺度目标检测中，不同尺度的特征映射是否对应于不同的抽象层次？为什么？
1. 在 *小节《13.5 多尺度目标检测》中多尺度锚框部分*中的实验里的第一个尺度（`fmap_w=4, fmap_h=4`）下，生成可能重叠的均匀分布的锚框。
1. 给定形状为$1 \times c \times h \times w$的特征图变量，其中$c$、$h$和$w$分别是特征图的通道数、高度和宽度。怎样才能将这个变量转换为锚框类别和偏移量？输出的形状是什么？

关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2947)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2948)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11805)。
