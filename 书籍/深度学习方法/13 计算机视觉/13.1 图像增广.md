 ...
 *小节《7.1 深度卷积神经网络（AlexNet）.md》中*提到过大型数据集是成功应用深度神经网络的先决条件。
...
```python
d2l.set_figsize()
img = image.imread('C:/Users/17100/Desktop/知识库/深度学习方法/img/cat1.jpg')
...
#@tab pytorch, paddle
d2l.set_figsize()
img = d2l.Image.open('C:/Users/17100/Desktop/知识库/深度学习方法/img/cat1.jpg')
d2l.plt.imshow(img);
```
...
在 *小节《6.5 汇聚层.md》中*中，我们解释了汇聚层可以降低卷积层对目标位置的敏感性。
...
*若使用mxnet实现*
接下来，我们定义了一个辅助函数，以便于读取图像和应用图像增广。Gluon数据集提供的`transform_first`函数将图像增广应用于每个训练样本的第一个元素（由图像和标签组成），即应用在图像上。有关`DataLoader`的详细介绍，请参阅 *小节《3.5 图像分类数据集.md》中*。

*若使用pytorch实现*
接下来，我们[**定义一个辅助函数，以便于读取图像和应用图像增广**]。PyTorch数据集提供的`transform`参数应用图像增广来转化图像。有关`DataLoader`的详细介绍，请参阅 *小节《3.5 图像分类数据集.md》中*。
...
我们在CIFAR-10数据集上训练 *小节《7.6 残差网络（ResNet）.md》中*中的ResNet-18模型。
回想一下 *小节《12.6 多GPU的简洁实现.md》中*中对多GPU训练的介绍。
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2828)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2829)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11801)。
