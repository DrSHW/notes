...
现在我们已经了解了word2vec模型的技术细节和大致的训练方法，让我们来看看它们的实现。具体地说，我们将以 *小节《16.1 词嵌入（word2vec）.md》中*的跳元模型和 *小节《16.2 近似训练.md》中*的负采样为例。本节从用于预训练词嵌入模型的数据集开始：数据的原始格式将被转换为可以在训练期间迭代的小批量。
...
$$
 P(w_i) = \max\left(1 - \sqrt{\frac{t}{f(w_i)}}, 0\right),\tag{16.3.1}
$$
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/5734)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/5735)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11816)。
