...
如 图 2.4.1 所示，内接多边形的等长边越多，就越接近圆。
这个过程也被称为*逼近法*（method of exhaustion）。

<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/polygon-circle.svg" alt="用逼近法求圆的面积" style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 2.4.1 用逼近法求圆的面积</p>
2000多年后，微积分的另一支，*微分*（differential calculus）被发明出来。
在微分学最重要的应用是优化问题，即考虑如何把事情做到最好。
正如在 *小节《2.3 线性代数》中范数和目标部分*中讨论的那样，
这种问题在深度学习中是无处不在的。
...
我们可以将 公式 2.4.1 中的导数$f'(x)$解释为$f(x)$相对于$x$的*瞬时*（instantaneous）变化率。
...
[**通过令$x=1$并让$h$接近$0$，**] 公式 2.4.1 中(**$\frac{f(x+h)-f(x)}{h}$的数值结果接近$2$**)。
...
$$
 \frac{\partial y}{\partial x_i} = \lim_{h \rightarrow 0} \frac{f(x_1, \ldots, x_{i-1}, x_i+h, x_{i+1}, \ldots, x_n) - f(x_1, \ldots, x_i, \ldots, x_n)}{h}.\tag{2.4.7}
$$
...
$$
\nabla_{\mathbf{x}} f(\mathbf{x}) = \bigg[\frac{\partial f(\mathbf{x})}{\partial x_1}, \frac{\partial f(\mathbf{x})}{\partial x_2}, \ldots, \frac{\partial f(\mathbf{x})}{\partial x_n}\bigg]^\top,\tag{2.4.9}
$$
...
$$
\frac{\partial y}{\partial x_i} = \frac{\partial y}{\partial u_1} \frac{\partial u_1}{\partial x_i} + \frac{\partial y}{\partial u_2} \frac{\partial u_2}{\partial x_i} + \cdots + \frac{\partial y}{\partial u_m} \frac{\partial u_m}{\partial x_i}\tag{2.4.11}
$$
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1755)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1756)。

关于`tensorflow`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1754)。
