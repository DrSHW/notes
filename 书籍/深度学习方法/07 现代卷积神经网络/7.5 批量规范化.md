...
再结合在 *小节《7.6 残差网络（ResNet）.md》中*中将介绍的残差块，批量规范化使得研究人员能够训练100层以上的网络。
...
回想一下我们应用多层感知机来预测房价的例子（ *小节《4.10 实战Kaggle比赛：预测房价.md》中*）。
...
$$
在 公式 7.5.1 中，$\hat{\boldsymbol{\mu}}_\mathcal{B}$是小批量$\mathcal{B}$的样本均值，$\hat{\boldsymbol{\sigma}}_\mathcal{B}$是小批量$\mathcal{B}$的样本标准差。
...
从形式上来看，我们计算出 公式 7.5.1 中的$\hat{\boldsymbol{\mu}}_\mathcal{B}$和${\hat{\boldsymbol{\sigma}}_\mathcal{B}}$，如下所示：
...
为了更好理解如何[**应用`BatchNorm`**]，下面我们将其应用(**于LeNet模型**)（ *小节《6.6 卷积神经网络（LeNet）.md》中*）。
...
这个代码与我们第一次训练LeNet（ *小节《6.6 卷积神经网络（LeNet）.md》中*）时几乎完全相同，主要区别在于学习率大得多。
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1876)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1874)。

关于`tensorflow`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1875)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11792)。
