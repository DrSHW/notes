...
有趣的是，在网络的最底层，模型学习到了一些类似于传统滤波器的特征抽取器。 图 7.1.1 是从AlexNet论文 复制的，描述了底层图像特征。

<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/filters.png" alt="AlexNet第一层学习到的特征抽取器。" style="width:`400px`
;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 7.1.1 AlexNet第一层学习到的特征抽取器。</p>
...
AlexNet和LeNet的架构非常相似，如 图 7.1.2 所示。
注意，本书在这里提供的是一个稍微精简版本的AlexNet，去除了当年需要两个小型GPU同时运算的设计特点。

<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/alexnet.svg" alt="从LeNet（左）到AlexNet（右）" style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 7.1.2 从LeNet（左）到AlexNet（右）</p>
...
AlexNet通过暂退法（ *小节《4.6 暂退法（Dropout）.md》中*）控制全连接层的模型复杂度，而LeNet只使用了权重衰减。
为了进一步扩充数据，AlexNet在训练时增加了大量的图像增强数据，如翻转、裁切和变色。
这使得模型更健壮，更大的样本量有效地减少了过拟合。
在 *小节《13.1 图像增广.md》中*中更详细地讨论数据扩增。
...
现在AlexNet可以开始被训练了。与 *小节《6.6 卷积神经网络（LeNet）.md》中*中的LeNet相比，这里的主要变化是使用更小的学习速率训练，这是因为网络更深更广、图像分辨率更高，训练卷积神经网络就更昂贵。
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1864)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1863)。

关于`tensorflow`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1862)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11788)。
