...
<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/inception.svg" alt="Inception块的架构。" style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 7.4.1 Inception块的架构。</p>
...
如 图 7.4.2 所示，GoogLeNet一共使用9个Inception块和全局平均汇聚层的堆叠来生成其估计值。Inception块之间的最大汇聚层可降低维度。
第一个模块类似于AlexNet和LeNet，Inception块的组合从VGG继承，全局平均汇聚层避免了在最后使用全连接层。

<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/inception-full.svg" alt="GoogLeNet架构。" style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 7.4.2 GoogLeNet架构。</p>
...
1. GoogLeNet有一些后续版本。尝试实现并运行它们，然后观察实验结果。这些后续版本包括：
    * 添加批量规范化层 （batch normalization），在 *小节《7.5 批量规范化.md》中*中将介绍；
    * 对Inception模块进行调整 ；
    * 使用标签平滑（label smoothing）进行模型正则化 ；
    * 加入残差连接 。（ *小节《7.6 残差网络（ResNet）.md》中*将介绍）。
1. 使用GoogLeNet的最小图像大小是多少？
1. 将AlexNet、VGG和NiN的模型参数大小与GoogLeNet进行比较。后两个网络架构是如何显著减少模型参数大小的？

关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1873)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1871)。

关于`tensorflow`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1872)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11791)。
