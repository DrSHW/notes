...
正如在 *小节《11.6 动量法.md》中*中那样，我们可以根据其特征分解$\mathbf{Q} = \mathbf{U}^\top \boldsymbol{\Lambda} \mathbf{U}$重写这个问题，来得到一个简化得多的问题，使每个坐标都可以单独解出：
...
与 *小节《11.5 小批量随机梯度下降.md》中*一节中的实验相比，这里使用更大的学习率来训练模型。
...
* 在深度学习问题上，AdaGrad算法有时在降低学习率方面可能过于剧烈。我们将在 *小节《11.10 Adam算法.md》中*一节讨论缓解这种情况的策略。
...
1. 尝试对适当的深度网络使用AdaGrad算法，例如，*小节《6.6 卷积神经网络（LeNet）.md》中*中应用于Fashion-MNIST的深度网络。
1. 要如何修改AdaGrad算法，才能使其在学习率方面的衰减不那么激进？

关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/4318)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/4319)。

关于`tensorflow`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/4320)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11852)。