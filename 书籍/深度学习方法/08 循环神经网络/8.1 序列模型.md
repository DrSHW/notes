...
为了简单起见，我们以 图 8.1.1 所示的股票价格（富时100指数）为例。

<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/ftse100.png" alt="近30年的富时100指数" style="width:`400px`
;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 8.1.1 近30年的富时100指数</p>
...
例如在 *小节《3.3 线性回归的简洁实现.md》中*中训练的模型。
仅有一个主要问题：输入数据的数量，
...
<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/sequence-model.svg" alt="隐变量自回归模型" style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 8.1.2 隐变量自回归模型</p>
...
$$
P(x_1, \ldots, x_T) = \prod_{t=1}^T P(x_t \mid x_{t-1}, \ldots, x_1).\tag{8.1.2}
$$
而不是连续的数字，则上述的考虑仍然有效。
唯一的差别是，对于离散的对象，
...
（我们将在 *小节《9.4 双向循环神经网络.md》中*再次遇到），
...
现在，准备[**训练模型**]了。实现下面的训练代码的方式与前面几节（如 *小节《3.3 线性回归的简洁实现.md》中*）中的循环训练基本相同。因此，我们不会深入探讨太多细节。
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2090)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2091)。

关于`tensorflow`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2092)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11795)。
