...
以及需要对循环神经网络*分离梯度*。
例如，在 *小节《8.5 rnn-scratch.md》中*中，
我们在序列上调用了`detach`函数。
为了能够快速构建模型并了解其工作原理，
上面所说的这些概念都没有得到充分的解释。
...
输入表示为$x_t$，输出表示为$o_t$。
回想一下我们在 *小节《8.4 循环神经网络》中有隐状态的循环神经网络部分*中的讨论，
...
或者，我们可以在$\tau$步后截断
 公式 8.7.7 中的求和计算。
这是我们到目前为止一直在讨论的内容，
例如在 *小节《8.5 rnn-scratch.md》中*中分离梯度时。
这会带来真实梯度的*近似*，
...
<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/truncated-bptt.svg" alt="比较RNN中计算梯度的策略，3行自上而下分别为：随机截断、常规截断、完整计算" style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 8.7.1 比较RNN中计算梯度的策略，3行自上而下分别为：随机截断、常规截断、完整计算</p>
当基于循环神经网络使用通过时间反向传播
...
与 *小节《8.7 通过时间反向传播》中循环神经网络的梯度分析部分*中的分析不同，
...
<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/rnn-bptt.svg" alt="上图表示具有三个时间步的循环神经网络模型依赖关系的计算图。未着色的方框表示变量，着色的方框表示参数，圆表示运算符" style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 8.7.2 上图表示具有三个时间步的循环神经网络模型依赖关系的计算图。未着色的方框表示变量，着色的方框表示参数，圆表示运算符</p>
...
我们继续使用如 *小节《4.7 前向传播、反向传播和计算图.md》中*中
所述的$\text{prod}$运算符。
...
如 *小节《8.7 通过时间反向传播》中循环神经网络的梯度分析部分*中所述。
...
正如我们在 *小节《4.7 前向传播、反向传播和计算图.md》中*中所解释的那样，
