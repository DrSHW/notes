...
甚至是一个词元序列进行建模。
假设在单词级别对文本数据进行词元化，
我们可以依靠在 *小节《8.1 序列模型.md》中*中对序列模型的分析。
让我们从基本概率规则开始：

$$
P(x_1, x_2, \ldots, x_T) = \prod_{t=1}^T P(x_t  \mid  x_1, \ldots, x_{t-1}).\tag{8.3.2}
$$

$$
P(\text{deep}, \text{learning}, \text{is}, \text{fun}) =  P(\text{deep}) P(\text{learning}  \mid  \text{deep}) P(\text{is}  \mid  \text{deep}, \text{learning}) P(\text{fun}  \mid  \text{deep}, \text{learning}, \text{is}).\tag{8.3.3}
$$
...
$$
\hat{P}(\text{learning} \mid \text{deep}) = \frac{n(\text{deep, learning})}{n(\text{deep})},\tag{8.3.4}
$$
...
回想一下我们在 *小节《8.1 序列模型.md》中*中对马尔可夫模型的讨论，
并且将其应用于语言建模。
...
根据 *小节《8.2 文本预处理.md》中*中介绍的时光机器数据集构建词表，
...
在 *小节《8.1 序列模型.md》中*中我们以一种相当特别的方式做到了这一点：
...
<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/timemachine-5gram.svg" alt="分割文本时，不同的偏移量会导致不同的子序列" style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 8.3.1 分割文本时，不同的偏移量会导致不同的子序列</p>
事实上，他们都一样的好。
然而，如果我们只选择一个偏移量，
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2096)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2097)。

关于`tensorflow`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2098)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11797)。
