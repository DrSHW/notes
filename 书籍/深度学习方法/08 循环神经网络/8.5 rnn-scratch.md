...
本节将根据 *小节《8.4 循环神经网络.md》中*中的描述，
从头开始基于循环神经网络实现字符级语言模型。
这样的模型将在H.G.Wells的时光机器数据集上训练。
和前面 *小节《8.3 语言模型和数据集.md》中*中介绍过的一样，
我们先读取数据集。
...
它在 *小节《3.4 softmax回归》中分类问题部分*中介绍过。
...
它与我们训练 *小节《3.6 softmax回归的从零开始实现.md》中*模型的方式有三个不同之处。

1. 序列数据的不同采样方法（随机采样和顺序分区）将导致隐状态初始化的差异。
1. 我们在更新模型参数之前裁剪梯度。
   这样的操作的目的是，即使训练过程中某个点上发生了梯度爆炸，也能保证模型不会发散。
1. 我们用困惑度来评价模型。如 *小节《8.4 循环神经网络》中困惑度（Perplexity）部分*所述，
...
因此需要为每个迭代周期重新初始化隐状态。
与 *小节《3.6 softmax回归的从零开始实现.md》中*中的
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2102)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2103)。

关于`tensorflow`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2104)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11799)。
