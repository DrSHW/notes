...
利用 *小节《16.8 来自Transformers的双向编码器表示（BERT）.md》中*中实现的BERT模型和 *小节《16.9 用于预训练BERT的数据集.md》中*中从WikiText-2数据集生成的预训练样本，我们将在本节中在WikiText-2数据集上对BERT进行预训练。
...
def load_data_wiki(batch_size, max_len):
    ...
    Defined in *小节《16.9 用于预训练BERT的数据集》中生成遮蔽语言模型任务的数据部分*"""
...
通过调用上述两个辅助函数，下面的`train_bert`函数定义了在WikiText-2（`train_iter`）数据集上预训练BERT（`net`）的过程。训练BERT可能需要很长时间。以下函数的输入`num_steps`指定了训练的迭代步数，而不是像`train_ch13`函数那样指定训练的轮数（参见 *小节《13.1 图像增广.md》中*）。
...
考虑“a crane is flying”这句话。回想一下 *小节《16.8 来自Transformers的双向编码器表示（BERT）》中输入表示部分*中讨论的BERT的输入表示。插入特殊标记“&lt;cls&gt;”（用于分类）和“&lt;sep&gt;”（用于分隔）后，BERT输入序列的长度为6。因为零是“&lt;cls&gt;”词元，`encoded_text[:, 0, :]`是整个输入语句的BERT表示。为了评估一词多义词元“crane”，我们还打印出了该词元的BERT表示的前三个元素。
...
在 *章节《15 自然语言处理：应用》中*中，我们将为下游自然语言处理应用微调预训练的BERT模型。
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/5742)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/5743)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11824)。
