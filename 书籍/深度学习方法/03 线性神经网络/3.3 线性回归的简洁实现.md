...
在 *小节《3.2 线性回归的从零开始实现.md》中*中，我们只运用了：
（1）通过张量来进行数据存储和线性代数；
（2）通过自动微分来计算梯度。
 ...
 *小节《3.2 线性回归的从零开始实现.md》中*中的(**线性回归模型**)。

## 生成数据集

与 *小节《3.2 线性回归的从零开始实现.md》中*中类似，我们首先[**生成数据集**]。
...
使用`data_iter`的方式与我们在 *小节《3.2 线性回归的从零开始实现.md》中*中使用`data_iter`函数的方式相同。为了验证是否正常工作，让我们读取并打印第一个小批量样本。
与 *小节《3.2 线性回归的从零开始实现.md》中*不同，这里我们使用`iter`构造Python迭代器，并使用`next`从迭代器中获取第一项。
...
当我们在 *小节《3.2 线性回归的从零开始实现.md》中*中实现线性回归时，
...
$$
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1782)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1781)。

关于`tensorflow`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1780)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11690)。
