...
正如我们在 *小节《9.5 机器翻译与数据集.md》中*中所讨论的，
机器翻译是序列转换模型的一个核心问题，
其输入和输出都是长度可变的序列。
为了处理这种类型的输入和输出，
我们可以设计一个包含两个主要组件的架构：
...
<img src="C:/Users/17100/Desktop/知识库/深度学习方法/img/encoder-decoder.svg" alt="编码器-解码器架构" style="width:;display:block;margin-left:auto;margin-right:auto;background-color: white;">
<p style="text-align:center">图 9.6.1 编码器-解码器架构</p>
...
这在 *小节《9.5 机器翻译与数据集》中加载数据集部分*中进行了解释。
为了逐个地生成长度可变的词元序列，
解码器在每个时间步都会将输入
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2780)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/2779)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11837)。