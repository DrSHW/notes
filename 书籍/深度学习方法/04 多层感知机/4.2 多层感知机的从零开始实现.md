...
我们已经在 *小节《4.1 多层感知机.md》中*中描述了多层感知机（MLP），
现在让我们尝试自己实现一个多层感知机。
为了与之前softmax回归（ *小节《3.6 softmax回归的从零开始实现.md》中* ）
获得的结果进行比较，
...
由于我们已经从零实现过softmax函数（ *小节《3.6 softmax回归的从零开始实现.md》中*），
因此在这里我们直接使用高级API中的内置函数来计算softmax和交叉熵损失。
回想一下我们之前在 *小节《3.7 softmax回归的简洁实现》中重新审视Softmax的实现部分*中
对这些复杂问题的讨论。
...
可以直接调用`d2l`包的`train_ch3`函数（参见 *小节《3.6 softmax回归的从零开始实现.md》中* ），
...
关于`mxnet`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1800)。

关于`pytorch`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1804)。

关于`tensorflow`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/1798)。

关于`paddle`的讨论，请参见[Disscussions](https://discuss.d2l.ai/t/11769)。
